...
class MailMail(models.Model):
    ...
    def _send(self, auto_commit=False, raise_exception=False, smtp_session=None):
        ...
        for mail_id in self.ids:
            ...
            if mail and mail.model and mail.res_id and group_portal:
                    ...
                    if partners_len > 1:
                        ...
                        if cc_internal:
                            partners = partners_obj.filtered(
                                lambda x: x.id not in user_partner_ids and (not x.user_ids or (x.user_ids and x.user_ids[0].show_in_cc and group_internal in x.user_ids.groups_id) or group_portal in x.user_ids.groups_id)
                            )
                        else:
                            partners = partners_obj.filtered(
                                lambda x: x.id not in user_partner_ids and (not x.user_ids or group_portal in x.user_ids.groups_id)
